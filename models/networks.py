...
def define_net(input_nc, output_nc, net, gpu_ids=[], init_type='normal', init_gain=0.02):
    if net == 'unet':
        net = UNet(in_channels=input_nc, n_classes=output_nc)
    elif net == 'axialunet':
        net = AxialUNet(in_channels=input_nc, n_classes=output_nc)
    return init_net(net, init_type, init_gain=init_gain, gpu_ids=gpu_ids)
...
class UNet(nn.Module):
    ...
    def __init__(self, in_channels, n_classes, base_n_filter=8):
        ...
        self.final_conv = nn.Conv3d(self.base_n_filter * 8, 64, (1, 5, 5))
...
class AxialAttention3D(nn.Module):
    def __init__(self, n_features, emb_shape, heads, dim_heads, bn=True, sum_axial_out=True, residual_attention=False):
...
class AxialUNet(nn.Module):
    ...
    def __init__(self, in_channels, n_classes, base_n_filter=8):
        ...
        self.ds3_1x1_conv3d = nn.Conv3d(self.base_n_filter * 4, self.n_classes, kernel_size=1, stride=1, padding=0,
                                        bias=False)
        self.sigmoid = nn.Sigmoid()
        # self.axial_attention_down = AxialAttention3D(n_features=128,
        #                                               emb_shape =(128, 128, 128),
        #                                               heads=1,
        #                                               dim_heads=32,
        #                                               sum_axial_out=True,
        #                                               residual_attention=True)
        self.axial_attention_down = AxialAttention(dim=128,
                                                   heads=4,
                                                   dim_index=2,
                                                   num_dimensions=3)
    ...
    def forward(self, x):
        ...
        out = self.lrelu(out)
        out = self.conv3d_c1_2(out)
        out = self.axial_attention_down(out)
        out = self.dropout3d(out)
        out = self.lrelu_conv_c1(out)
        # Element Wise Summation
        out += residual_1
